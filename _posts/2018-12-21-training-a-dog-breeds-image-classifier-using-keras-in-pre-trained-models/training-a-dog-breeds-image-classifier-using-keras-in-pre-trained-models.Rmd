...
```{r}
extract_features <- function(image_generator, steps = 10){
  
  xception_out <- dim(xception_model$output)[[2]]
  inception_out <- dim(inception_model$output)[[2]]
  n_features <- xception_out + inception_out
  
  x <- matrix(NA, steps * batch_size, n_features)
  y <- matrix(NA, steps * batch_size, length(classes))
  
  pb <- progress_estimated(n = steps)
  for(i in 1:steps){
    pb$tick()$print()
...
data_tr <- extract_features(train_gen,
                            steps = ceiling(train_gen$n/batch_size * 2))
...
```{r}
data_vl <- extract_features(valid_gen, steps = ceiling(valid_gen$n/batch_size))
xvalid <- data_vl$x
yvalid <- data_vl$y
rm(data_vl)
```
...
custom_generator <- function(generator){
  
  function(){
    data <- generator_next(generator)
    input <- data
